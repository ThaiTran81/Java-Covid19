...
@DISTRICT NVARCHAR(50) = NULL, @VILLAGE NVARCHAR(50) = NULL, @ID_QUARATINE INT = NULL, @RELATED VARCHAR(15) = NULL, @F_KIND VARCHAR(5)=NULL)
AS
BEGIN
  DECLARE @ID_BANK INT;
  SET @ID_BANK = NULL;
  IF @ROLE = 2
  BEGIN
    INSERT INTO PAYMENT (BALANCE)
      VALUES (20000000);
    SET @ID_BANK = (SELECT
        SCOPE_IDENTITY());
  END
  INSERT INTO PROFILE (ID, FULLNAME, PHONENUMBER, DOB, GENDER, PROVINE, DISTRICT, VILLAGE, ID_BANK, ID_QUARATINE, F_STATUS, RELATED_USER)
    VALUES (@ID, @FULLNAME, @PHONENUMBER, @DOB, @GENDER, @PROVINE, @DISTRICT, @VILLAGE, @ID_BANK, @ID_QUARATINE, @F_KIND, @RELATED);

  IF @ROLE = 2
  BEGIN
    INSERT INTO DEBT (USER_ID, DEBT)
      VALUES (@ID, 0);
  END
...
SELECT p.ID, p.FULLNAME, p.DOB, p.F_STATUS, p.ID_QUARATINE
FROM PROFILE p 
